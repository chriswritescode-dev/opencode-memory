  ...
  try {
    ...
    if (argv.name) {
      ...
      if (restartableLoops.length === 0) {
        console.log('')
        console.log('No restartable memory loops found.')
        console.log('')
        return
      }
      if (restartableLoops.length === 1) {
        ...
        console.log('')
        console.log('Multiple restartable memory loops. Please specify which one to restart:')
        console.log('')
        for (const l of restartableLoops) {
          console.log(`  - ${l.state.worktreeName}`)
        }
        console.log('')
        console.log("Run 'ocm-mem restart <name>' to restart a specific loop.")
        console.log('')
        process.exit(1)
      }
    }
    ...
    if (state.terminationReason === 'completed') {
      console.log('')
      console.log(`Memory loop "${state.worktreeName}" completed successfully and cannot be restarted.`)
      console.log('')
      process.exit(1)
    }
    ...
    if (state.active && !argv.force) {
      console.log('')
      console.log(`Memory Loop to Force Restart:`)
      ...
      const shouldProceed = await confirm(`Force restart active memory loop '${state.worktreeName}'`)
    ...
    console.log('')
    console.log(`Restarted memory loop "${state.worktreeName}"`)
    console.log('')
...
export function help(): void {
  console.log(`
Restart a memory loop

Usage:
  ocm-mem restart [name] [options]
