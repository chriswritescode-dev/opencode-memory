...
describe('SandboxManager', () => {
  ...
  describe('start', () => {
    ...
    test('returns early when container already running', async () => {
      const mockDocker = createMockDockerService()
      mockDocker.setRunning('ocm-sandbox-test', true)
      const logger = createMockLogger()
      const manager = createSandboxManager(
        mockDocker as unknown as DockerService,
        { image: 'ocm-sandbox:latest' },
      ...
      expect(result).toEqual({ containerName: 'ocm-sandbox-test' })
    })
    ...
    test('creates container and populates active map', async () => {
      ...
      expect(active).not.toBeNull()
      expect(active?.containerName).toBe('ocm-sandbox-test')
    })
  })
  ...
  describe('stop', () => {
    test('removes container and clears active map', async () => {
      ...
      expect(mockDocker.getRemoveContainerCalls()).toContain('ocm-sandbox-test')
    ...
    test('uses containerName fallback when not in active map', async () => {
      ...
      expect(mockDocker.getRemoveContainerCalls()).toContain('ocm-sandbox-unknown')
    })
  })
  ...
  describe('cleanupOrphans additional', () => {
    ...
    test('continues cleanup when removal fails', async () => {
      const mockDocker = createMockDockerService()
      mockDocker.setContainers(['ocm-sandbox-first', 'ocm-sandbox-second'])
      mockDocker.setRemoveThrow(true)
      const logger = createMockLogger()
      const manager = createSandboxManager(
        mockDocker as unknown as DockerService,
      ...
      const calls = mockDocker.getRemoveContainerCalls()
      expect(calls).toContain('ocm-sandbox-first')
      expect(calls).toContain('ocm-sandbox-second')
    })
  })
})
